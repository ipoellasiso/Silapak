...
class PdfUploadController extends Controller
{
    ...
    public function upload(Request $request)
    {
        ...
        foreach ($lines as $line) {
            ...
            // === DETEKSI KEGIATAN ===
            // if (preg_match('/^\d*\s*(5[\.\s]\d{2}[\.\s]\d{2}[\.\s]\d{1,2}[\.\s]\d{2}[\.\s]\d{4})\s*[-–]?\s*(.+)$/u', $line, $m)) {
            //     $lastKegiatan = ucwords(strtolower(trim($m[2])));
            //     $lastSubKegiatan = null; // reset sub kegiatan saat ganti kegiatan
            //     continue;
            // }
            ...
            // === DETEKSI SUB KEGIATAN ===
            // if (preg_match('/^\d*\s*(5[\.\s]\d{2}[\.\s]\d{2}[\.\s]\d{1,2}[\.\s]\d{2})\s*[-–]?\s*(.+)$/u', $line, $m)) {
            //     $lastSubKegiatan = ucwords(strtolower(trim($m[2])));
            //     continue;
            // }
        ...
        // Simpan rekening beserta kegiatan & subkegiatan terakhir
        // $rekening_belanja[] = [
        //     'kode' => $kode,
        //     'uraian' => $uraian,
        //     'nilai' => $nilai,
        //     'kegiatan' => $lastKegiatan,
        //     'sub_kegiatan' => $lastSubKegiatan
        // ];

        Log::info('=== DEBUG PARSING ===', [
            'kegiatan' => $kegiatan,
        ...
        // === GABUNGKAN KEGIATAN DAN SUB KEGIATAN KE REKENING BELANJA ===
        $gabungan_belanja = [];
        $lastKegiatan = null;
        $lastSub = null;

        foreach ($rekening_belanja as $rek) {
            $rekKodeDigits = preg_replace('/[^0-9]/', '', $rek['kode']);

            // 🧭 Cari kegiatan terdekat
            foreach ($kegiatan as $keg) {
                $kegKodeDigits = preg_replace('/[^0-9]/', '', $keg['kode']);
                // kalau awalnya sama (contoh: 50201206...) maka dianggap match
                if (strpos($rekKodeDigits, $kegKodeDigits) === 0) {
                    $lastKegiatan = $keg['uraian'];
                }
            }

            // 🧭 Cari sub kegiatan terdekat (paling panjang dan mirip)
            $bestMatchLength = 0;
            $lastSub = null;
            foreach ($sub_kegiatan as $sub) {
                $subKodeDigits = preg_replace('/[^0-9]/', '', $sub['kode']);

                // hitung kesamaan prefix (misalnya cocok sampai 9 digit pertama)
                $prefixMatch = 0;
                for ($i = 0; $i < strlen($rekKodeDigits) && $i < strlen($subKodeDigits); $i++) {
                    if ($rekKodeDigits[$i] === $subKodeDigits[$i]) {
                        $prefixMatch++;
                    } else break;
                }

                if ($prefixMatch > $bestMatchLength) {
                    $bestMatchLength = $prefixMatch;
                    $lastSub = $sub['uraian'];
                }
            }

            // Jika tidak ada sub kegiatan cocok, fallback ke kegiatan
            if (!$lastSub && $lastKegiatan) {
                $lastSub = $lastKegiatan;
            }
            ...
            $gabungan_belanja[] = [
                ...
                'uraian' => $rek['uraian'],
                'nilai' => $rek['nilai'],
                'kegiatan' => $lastKegiatan ?? null,
                'sub_kegiatan' => $lastSub ?? null
            ];
        }
        ...
        foreach ($gabungan_belanja as $rek) {
            BelanjalsguModel::create([
                'id_sp2d'       => $sp2d->idhalaman,
                'norekening'    => $rek['kode'],
                'uraian'        => $rek['uraian'],
                'nilai'         => $rek['nilai'],
                'kegiatan'      => $rek['kegiatan'] ?? null,
                'sub_kegiatan'  => $rek['sub_kegiatan'] ?? null,
                'status1'       => 'Draft',
                'status2'       => 'Belum Diverifikasi',
            ]);
        }
